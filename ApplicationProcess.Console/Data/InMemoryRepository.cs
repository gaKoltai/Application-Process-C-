...
namespace Codecool.ApplicationProcess.Data
{
    ...
    public class InMemoryRepository : IApplicationRepository
    {
        ...
        {
            return _mentors.Where(mentor => mentor.City.Equals(city));
        }

        /// <inheritdoc/>
        public IEnumerable<Mentor> GetAllMentorWhomFavoriteLanguage(string language)
        {
            return _mentors.Where(mentor => mentor.ProgrammingLanguage.Equals(language));
        }

        /// <inheritdoc/>
        public IEnumerable<Applicant> GetApplicantsOf(string contactMentorName)
        {
            var applicants = from applicant in _applicants
                join application in _applications on applicant equals application.Applicant
                where application.Mentor.Nickname == contactMentorName
                select applicant;
        ...
        {
            return _applicants.Select(applicant => applicant.Email);
        }
